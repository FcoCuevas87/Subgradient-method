...
fwag<- function(d,par,par2,j){
  return(2*par[j]*exp(-d/par2[j])*(1*(d<=0.2) + 0* (d>=0.2)))
}
fwag2<- function(d,par,par2,j){
  return(2*exp(-d/par2[j])*(1*(d<=0.2) + 0* (d>=0.2)))
}
#integral1 
...
integrandM<-function(d,par,par2,j){
  w=exp(cove(d,par,par2))*fwag2(d,par,par2,j)
  return(w)
}
...
jacob<-function(par,par2){
  ...
  for (i in 1:length(par)){
    for (j in 1:length(par) ){
      ...
      if(i==j){
        temp=temp+sum(fwag2(P$d,par,par2,i))-(intensity(X))^2*(1/mc)^2*sum(integrandM(dU$d,par,par2,i))
      }
      jac[i,j]=temp
    }
  }}
...
par=c(sqrt(0),sqrt(0.1))
#phi
par2=c(0.05,0.15)
alp=0.000001
lam=0
print(jacob(par,par2))
for (k in 1:200){
  para=par
  jac=djacob(par,par2)
  jac=jac^{-1}
...
for (k in 1:200){
  ...
  #funcion jacob esta dando problemas
  #jac=jacob(par,par2)
  jac=solve(jac)
